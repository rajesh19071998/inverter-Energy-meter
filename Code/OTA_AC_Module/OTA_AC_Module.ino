...
void DC_Energy_Reset(uint8_t slaveAddr) {    // DC_Energy_Reset(pzemSlaveAddr);
  static uint8_t reset_cmd = 0x42; 
  uint16_t u16CRC = 0xFFFF;                                                                         /* declare CRC check 16 bits*/
  u16CRC = crc16_update(u16CRC, slaveAddr);                                                         // Calculate the crc16 over the 6bytes to be send
  u16CRC = crc16_update(u16CRC, reset_cmd);
      
  Serial.println("Change Reset address");
  DC_Serial.write(slaveAddr); //these whole process code sequence refer to manual
  DC_Serial.write(reset_cmd);
...
void MIT_APP() {
  ...
  // Check if a client has connected
   ...
   if ( request.indexOf("ACV") > 0 )  {MIT_flag = 1; sprintf(DATA_1, "%f%s", AC_voltage," V");}
    ...
    else if  ( request.indexOf("WNQ") > 0 ) { MIT_flag = 1; Serial.println(request);sprintf(DATA_1, "%s%s","WNA ",AP_NameChar);Serial.print("AP_NameChar : ");Serial.println(AP_NameChar);}
    else if  ( request.indexOf("WPQ") > 0 ) { MIT_flag = 1; Serial.println(request);sprintf(DATA_1, "%s%s","WPA ",WiFiPassword);Serial.print("WiFiPassword : ");Serial.println(WiFiPassword); }
    else if  ( request.indexOf("HNQ") > 0 ) { MIT_flag = 1; Serial.println(request);sprintf(DATA_1, "%s%s","HNA ",ssid);Serial.print("ssid : ");Serial.println(ssid); }
    else if  ( request.indexOf("HPQ") > 0 ) { MIT_flag = 1; Serial.println(request);sprintf(DATA_1, "%s%s","HPA ",password);Serial.print("password : ");Serial.println(password); }
    else if  ( request.indexOf("STA") > 0 ) { MIT_flag = 0;Serial.println(request); Last_OTA_Status(); }
    
    else if  ( request.indexOf("WN") > 0 ) { MIT_flag = 1;if(DEBUG) Serial.println(request); Wifi_update(request,1); }
    else if  ( request.indexOf("WP") > 0 ) { MIT_flag = 1;if(DEBUG) Serial.println(request); Wifi_update(request,2); }
    else if  ( request.indexOf("HN") > 0 ) { MIT_flag = 1;if(DEBUG) Serial.println(request); Wifi_update(request,3); }
    else if  ( request.indexOf("HP") > 0 ) { MIT_flag = 1;if(DEBUG) Serial.println(request); Wifi_update(request,4); }
    else if  ( request.indexOf("RST") > 0 ) { MIT_flag = 1;if(DEBUG) Serial.println(request);delay(5000);ESP.restart(); }
...
void Reset_AC_Energy()
{
  bool response;
   response = pzem.resetEnergy();
   if(response)
   Serial.println("Energy Reset was Done Successfully");

   else
   Serial.println("Energy Reset was Failed");
 }
